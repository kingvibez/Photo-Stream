...
public class PhotoStreamFragment extends Fragment {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		//If mobile device is post honeycomb, we can use action views
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			//get the search menu item
			MenuItem searchItem = menu.findItem(R.id.menu_item_search);
			//get that menu item's action view, and cast it to a search view
			SearchView searchView = (SearchView)searchItem.getActionView();
			
			//Get the SearchManager system service
			SearchManager searchManager = (SearchManager)getActivity().getSystemService(Context.SEARCH_SERVICE);
			ComponentName name = getActivity().getComponentName();
			//Get the searchable info from searchManager by plugging in your app name
			SearchableInfo searchInfo = searchManager.getSearchableInfo(name);
			
			//finally, set the search info on your previously empty search view
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			case R.id.menu_item_search:
				...
				else getActivity().onSearchRequested();
				return true;
